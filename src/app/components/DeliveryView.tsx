import { Bike, Clock, CreditCard, Package } from 'lucide-react';
...
export function DeliveryView({ orders, onOrderClick }: DeliveryViewProps) {
  if (orders.length === 0) {
    return (
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#444]">{orders.length} pedido{orders.length !== 1 ? 's' : ''} en camino</p>
          </div>
        ...
        {orders.map((order) => {
          ...
          return (
            <button
              ...
              className="bg-[#111111] border border-[#1e1e1e] hover:border-[#3b82f6]/60 rounded-2xl overflow-hidden text-left transition-all duration-250 hover:shadow-2xl hover:shadow-[#3b82f6]/8 group"
            >
              ...
              <div className="relative bg-gradient-to-br from-[#08111a] via-[#040e18] to-[#0d0d0d] px-5 py-6 text-center overflow-hidden">
                ...
                <div className="relative">
                  ...
                  <div className="text-[64px] leading-none text-[#f0f0f0] group-hover:text-[#3b82f6] transition-colors duration-200" style={{ fontVariantNumeric: 'tabular-nums' }}>
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center border-t border-[#1a1a1a] pt-3 mb-3">
                  ...
                  <span className="text-[#3b82f6] text-lg">{formatPrice(order.total)}</span>
                </div>

                {/* CTA */}
                <div className="bg-[#3b82f6]/8 border border-[#3b82f6]/15 group-hover:bg-[#3b82f6] group-hover:border-[#3b82f6] rounded-xl py-2.5 text-center transition-all duration-200">
                  <span className="text-[#3b82f6] group-hover:text-white text-xs flex items-center justify-center gap-2 tracking-wide uppercase">
                    <CreditCard size={13} />
                    Cobrar Pedido {order.deliveryNumber}
                  </span>
                </div>
              </div>
            </button>
